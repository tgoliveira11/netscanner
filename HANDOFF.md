...
PERSISTENCE=memory GATEWAY_PORT=4013 GATEWAY_HOST=127.0.0.1 NODE_ENV=production \
  PFSENSE_URL="https://192.168.51.1" PFSENSE_API_KEY="<key>" FINGERBANK_API_KEY="<key>" \
...
#        -H 'Origin: http://127.0.0.1:4013' -d '{"cidr":"192.168.51.0/24","scanType":"quick"}'
...
- **Router/firewall:** pfSense CE 2.8.1 at **`192.168.51.1`** (FreeBSD; the `.1` box is an
  Intel N5105 mini-PC — NIC OUI "S-Bluetech"). REST API package installed.
- **VLANs:** LAN_MAIN `192.168.51.x`, LAN_GUEST `192.168.52.x`, LAN_IOT `192.168.60.x`,
  LAN_INFRA `192.168.40.x`, plus TRUNK/WAN. The agent host is on `192.168.51.x`.
- Real devices seen: iPhones, an Apple Watch, MacBook Air/Pro, an Amazon speaker, many
  Tuya/Espressif IoT on LAN_IOT, Compal (`cbnre…`) ISP gateway/mesh nodes.
