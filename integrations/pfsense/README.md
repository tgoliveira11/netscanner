...
3. **Agent URL** — ex. `http://10.0.51.106:4000`
...
```bash
scp -r integrations/pfsense/pfSense-pkg-NetScanner root@10.0.51.1:/tmp/
```
...
```bash
/usr/local/bin/php /usr/local/pkg/configure-pfsense.php http://10.0.51.106:4000 SEU_TOKEN
/usr/local/bin/netscanner-collect
