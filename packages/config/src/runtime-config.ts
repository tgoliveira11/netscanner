...
export const CONFIG_FIELDS: ConfigFieldMeta[] = [
  ...
  { key: 'GATEWAY_HOST', label: 'Gateway host', description: 'Bind address. Use 0.0.0.0 for LAN-wide UI; 127.0.0.1 for localhost-only.', type: 'string', group: 'Gateway', restartRequired: true, help: 'Dedicated Linux boxes should bind 0.0.0.0. Mac UI-only peers can stay on 127.0.0.1 and redirect to the leader.' },
  ...
  { key: 'CLUSTER_ENABLED', label: 'Cluster enabled', description: 'UDP peer beacon and automatic leader election.', type: 'boolean', group: 'Cluster', restartRequired: true },
  { key: 'CLUSTER_BEACON_PORT', label: 'Cluster beacon port', description: 'UDP port for peer discovery beacons.', type: 'number', group: 'Cluster', restartRequired: true },
  { key: 'CLUSTER_PREFER_LEADER', label: 'Prefer leader', description: 'Hint that this agent should win election (dedicated boxes).', type: 'boolean', group: 'Cluster', restartRequired: true },
  { key: 'CLUSTER_DEDICATED', label: 'Dedicated appliance', description: 'Mark install as dedicated Linux hardware (strongest election preference).', type: 'boolean', group: 'Cluster', restartRequired: true },
  { key: 'AGENT_PROFILE', label: 'Agent profile', description: 'full, scan-only, or ui-only.', type: 'string', group: 'Cluster', restartRequired: true },
  { key: 'UI_ONLY', label: 'UI only', description: 'Disable elevated probes and control writes; follow the cluster leader.', type: 'boolean', group: 'Cluster', restartRequired: true },
  { key: 'MDNS_ENABLED', label: 'mDNS announce', description: 'Advertise netscanner.local when this agent is inventory leader.', type: 'boolean', group: 'Cluster', restartRequired: true },
  { key: 'MDNS_HOSTNAME', label: 'mDNS hostname', description: 'Hostname label without .local (default netscanner).', type: 'string', group: 'Cluster', restartRequired: true },
  { key: 'CLUSTER_UI_REDIRECT', label: 'Redirect UI to leader', description: 'Non-leaders redirect browser UI to the inventory leader.', type: 'boolean', group: 'Cluster', restartRequired: false },
  { key: 'CLUSTER_PEER_HOSTS', label: 'Cluster peer hosts', description: 'Comma-separated IPs for unicast UDP beacons (needed across VLANs).', type: 'string', group: 'Cluster', restartRequired: true },
  { key: 'CLUSTER_ADVERTISE_HOST', label: 'Advertise host', description: 'LAN IP/hostname peers should use to reach this agent HTTP API.', type: 'string', group: 'Cluster', restartRequired: true },
  { key: 'CLUSTER_CONTROL_ELIGIBLE', label: 'Control eligible', description: 'Allow this agent to become pfSense/Compal control leader.', type: 'boolean', group: 'Cluster', restartRequired: true },
  { key: 'CLOUD_SYNC_ENABLED', label: 'Cloud sync', description: 'Push observed inventory to self-host cloud near-realtime.', type: 'boolean', group: 'Cloud', restartRequired: true },
  { key: 'CLOUD_SYNC_URL', label: 'Cloud sync URL', description: 'Base URL of the self-host cloud service.', type: 'string', group: 'Cloud', restartRequired: true },
  { key: 'CLOUD_SYNC_TOKEN', label: 'Cloud sync token', description: 'Site token for cloud authentication.', type: 'secret', group: 'Cloud', restartRequired: true },
  { key: 'CLOUD_PII_CONSENT', label: 'Cloud PII consent', description: 'Allow uploading MACs, IPs, and hostnames to cloud.', type: 'boolean', group: 'Cloud', restartRequired: false },
  { key: 'CLOUD_SYNC_INTERVAL_MS', label: 'Cloud sync interval (ms)', description: 'How often to flush inventory events to cloud.', type: 'number', group: 'Cloud', restartRequired: false },
  ...
  { key: 'TUYA_ACCESS_ID', label: 'Tuya Access ID', description: 'IoT Platform Client ID for read-only Smart Life device names.', type: 'string', group: 'Integrations', restartRequired: true },
  { key: 'TUYA_ACCESS_SECRET', label: 'Tuya Access Secret', description: 'IoT Platform Client Secret (identity only — never used to control devices).', type: 'secret', group: 'Integrations', restartRequired: true },
  { key: 'TUYA_DATA_CENTER', label: 'Tuya data center', description: 'Must match Smart Life app region: us, us-e, eu, eu-w, cn, in, sg.', type: 'string', group: 'Integrations', restartRequired: true },
  { key: 'TUYA_SYNC_INTERVAL_MS', label: 'Tuya sync interval (ms)', description: 'How often to refresh the Tuya device list from the cloud (default 1h). Last catalog is kept in SQLite across restarts.', type: 'number', group: 'Integrations', restartRequired: false },
  ...
  { key: 'ROUTER_SCRAPE_TARGETS', label: 'Router scrape targets', description: 'One per line: OpenWrt `http://ip|openwrt|user|pass`, or Compal identity `compal|CLARO_xxxxxx|pass` (IP from discovery/MAC).', type: 'multiline', group: 'Integrations', restartRequired: true },
  ...
  { key: 'PASSIVE_DNS_ENABLED', label: 'DNS passive', description: 'tcpdump :53 for hostname hints + DNS intelligence.', type: 'boolean', group: 'Discovery', restartRequired: true },
  { key: 'DNS_INTEL_TOP_N', label: 'DNS intel top N', description: 'How many top domains to keep per device profile.', type: 'number', group: 'Discovery', restartRequired: false },
  { key: 'TSHARK_DEEP_ENABLED', label: 'tshark deep capture', description: 'TLS SNI / HTTP Host / richer DHCP via tshark (needs package).', type: 'boolean', group: 'Discovery', restartRequired: true },
  { key: 'LLDPD_ENABLED', label: 'lldpd neighbors', description: 'Poll lldpctl for L2 neighbors when lldpd is installed.', type: 'boolean', group: 'Discovery', restartRequired: true },
  { key: 'NETDISCOVER_ENABLED', label: 'Passive ARP', description: 'Passive ARP discovery of quiet hosts via tcpdump (needs root).', type: 'boolean', group: 'Discovery', restartRequired: true },
  { key: 'CVE_NVD_SYNC', label: 'CVE NVD sync', description: 'Refresh local CVE index from NVD subset (offline seed always available).', type: 'boolean', group: 'Discovery', restartRequired: false },
  { key: 'TRAFFIC_SPIKE_BPS', label: 'Traffic spike threshold (bps)', description: 'Emit TRAFFIC_SPIKE when rate exceeds this (default 50 Mbps).', type: 'number', group: 'Discovery', restartRequired: false },
  { key: 'NTOPNG_URL', label: 'ntopng URL', description: 'Optional ntopng API base for per-host traffic.', type: 'string', group: 'Integrations', restartRequired: true },
  { key: 'NTOPNG_TOKEN', label: 'ntopng token', description: 'ntopng API token.', type: 'secret', group: 'Integrations', restartRequired: true },
