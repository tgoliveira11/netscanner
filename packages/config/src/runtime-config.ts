...
export const CONFIG_FIELDS: ConfigFieldMeta[] = [
  ...
  { key: 'TOPOLOGY_VLAN_ORDER', label: 'Topology VLAN order', description: 'Comma-separated pfSense interface labels for vlan mode (e.g. LAN_INFRA,LAN_MAIN,LAN_GUEST).', type: 'string', group: 'Topology', restartRequired: false },
  { key: 'TOPOLOGY_WIRED_VLAN', label: 'Topology wired VLAN', description: 'pfSense interface label for the wired switch segment in vlan mode (e.g. LAN_INFRA).', type: 'string', group: 'Topology', restartRequired: false },
