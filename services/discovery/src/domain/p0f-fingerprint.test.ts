...
describe('p0f-fingerprint', () => {
  it('parses a verbose tcpdump SYN line', () => {
    const line =
      ...
      '192.168.51.100.54321 > 8.8.8.8.443: Flags [S], seq 1, win 65535, options [mss 1460,sackOK,TS val 1 ecr 0,nop,wscale 6], length 0';
    const parsed = parseTcpSynLine(line);
    expect(parsed?.ip).toBe('192.168.51.100');
