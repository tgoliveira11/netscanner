...
/** Claro/CBN Compal LuCI username: CLARO_ + last 3 MAC octets (hex, uppercase, no colons). */
export function deriveClaroUsername(mac: string): string | null {
  const hex = mac.replace(/[^0-9a-fA-F]/g, '').toUpperCase();
  if (hex.length < 6) return null;
  return `CLARO_${hex.slice(-6)}`;
}
