...
describe('parseDnsTcpdumpLine', () => {
  it('extracts LAN client and domain from a query line', () => {
    const line =
      '12:34:56.789 IP 192.168.51.100.54321 > 8.8.8.8.53: 12345+ A? device.tuyaus.com. (32)';
    expect(parseDnsTcpdumpLine(line)).toEqual({
      clientIp: '192.168.51.100',
  ...
  it('uses private IP when query direction is reversed', () => {
    const line = '12:34:56.789 IP 142.250.79.197.53 > 192.168.40.2.41000: 12345+ A? pool.ntp.org. (32)';
    expect(parseDnsTcpdumpLine(line)?.clientIp).toBe('192.168.40.2');
  });
});
