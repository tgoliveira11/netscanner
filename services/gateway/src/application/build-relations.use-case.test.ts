...
describe('buildDeviceRelations', () => {
    ...
    const phone = device({
      ...
      signals: {
        traffic: {
          ...
          connections: 2,
          topPeers: [
            { ip: '192.168.51.1', bytes: 5000 },
            { ip: '8.8.8.8', bytes: 9000 },
          ],
        },
      },
    });
    const gw = device({ id: 'gw', ip: '192.168.51.1', deviceType: 'firewall' });
  ...
  it('merges passive DNS queries per device IP', () => {
    const iot = device({ id: 'iot', ip: '192.168.60.50', hostname: 'plug' });
    const { edges } = buildDeviceRelations([iot], {
      passiveDnsByIp: (ip) =>
        ip === '192.168.60.50' ? ['device.tuyaus.com', 'device.tuyaus.com'] : [],
    });
