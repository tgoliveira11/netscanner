...
const vlanLabTopology = (): TopologyConfig => ({
  mode: 'vlan',
  vlanOrder: ['VLAN40', 'VLAN10', 'VLAN30', 'VLAN20'],
  wiredVlan: 'VLAN40',
  ...
  it('prefers collapsed multi-homed firewall over other .1 routers', () => {
    const devices = [
      ...
      baseDevice({
        ...
        deviceType: 'firewall',
        signals: {
          infrastructureIps: ['192.168.0.135', '10.0.40.1', '192.168.50.1', '10.0.52.1'],
        },
      }),
    ];
    expect(pickGateway(devices, [], { PFSENSE_URL: 'https://10.0.51.1' } as never)!.id).toBe('pfsense');
  });
  ...
  it('matches PFSENSE_URL against infrastructureIps on collapsed gateway', () => {
    const devices = [
      baseDevice({
        ...
        ip: '192.168.50.1',
        deviceType: 'firewall',
        signals: { infrastructureIps: ['192.168.50.1', '10.0.51.1', '10.0.52.1'] },
      }),
    ];
    expect(pickGateway(devices, [], { PFSENSE_URL: 'https://10.0.51.1' } as never)!.id).toBe('pfsense');
  });
...
describe('resolveClientAttachment', () => {
  ...
  const apByVlan = new Map([['VLAN10', ap]]);
  ...
  it('hangs non-SNMP wired VLAN10 clients under gateway in vlan mode', () => {
...
describe('collectVlans', () => {
  it('orders VLANs from config and skips WAN', () => {
    ...
    expect(
      collectVlans(edges, ['VLAN40', 'VLAN10', 'VLAN30', 'VLAN20']).map((v) => v.id),
    ).toEqual(['VLAN40', 'VLAN10', 'VLAN20']);
  });
});
...
describe('topologyRevision', () => {
  it('is stable for the same graph and changes when edges change', () => {
    const base = {
      gatewayId: 'gw',
      edges: [wiredEdge('a', 'gw', 'lan', { id: 'VLAN10', label: 'VLAN10' })],
      nodes: [{ id: 'gw', role: 'gateway' as const, tier: 0 }],
      vlans: [{ id: 'VLAN10', label: 'VLAN10' }],
      ssids: [],
    };
    ...
    const c = topologyRevision({
      ...base,
      edges: [...base.edges, wiredEdge('b', 'gw', 'lan', { id: 'VLAN10', label: 'VLAN10' })],
    });
    expect(c).not.toBe(a);
  });
});
