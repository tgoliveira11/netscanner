...
const CORE_VLAN_ORDER = ['LAN_INFRA', 'LAN_MAIN', 'LAN_GUEST', 'LAN_IOT'] as const;
 ...
 *     ├── LAN_INFRA  → wired switch/router
 *     ├── LAN_MAIN   → WiFi AP → clients
 *     ├── LAN_GUEST  → WiFi AP → clients
 *     ├── LAN_IOT    → WiFi AP → clients
...
export function pickWiredInfra(
  ...
  return (
    ...
    candidates.find((d) => resolveDeviceVlan(d).id === 'LAN_INFRA') ??
    candidates[0] ??
    null
  );
}
...
export function resolveClientAttachment(input: {
  ...
  if (isWired) {
    if (
      switchDevice &&
      (vlan.id === 'LAN_INFRA' || sameIpv4Slash24(device.ip, switchDevice.ip))
    ) {
      return {
        parentId: switchDevice.id,
        kind: 'wired',
