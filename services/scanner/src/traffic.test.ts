...
describe('parsePfStates', () => {
  it('aggregates bytes per private LAN IP from pfctl -vvs state output', () => {
    const out = [
      'all tcp 192.168.51.100:52344 -> 1.2.3.4:443       ESTABLISHED:ESTABLISHED',
      '   age 00:01:23, expires in 00:10:00, 10:20 pkts, 1000:5000 bytes, rule 5',
      'all udp 8.8.8.8:53 -> 192.168.51.100:41000       SINGLE:NO_TRAFFIC',
      '   age 00:00:02, expires in 00:00:30, 1:1 pkts, 60:120 bytes, rule 1',
    ].join('\n');
    const samples = parsePfStates(out);
    const dev = samples.find((s) => s.ip === '192.168.51.100');
  ...
  it('handles pfSense <- direction and NAT parentheses', () => {
    const out = [
      ...
      'em0 tcp 192.168.52.105:443 (192.168.51.1:12345) -> 8.8.8.8:443       ESTABLISHED:ESTABLISHED',
      '   age 00:00:10, 5:5 pkts, 100:200 bytes, rule 1',
    ].join('\n');
    const samples = parsePfStates(out);
    const lan = samples.find((s) => s.ip === '10.1.20.60');
    const nat = samples.find((s) => s.ip === '192.168.52.105');
  ...
  it('uses remote endpoint as peer, not NAT gateway in parentheses', () => {
    const out = [
      'em0 tcp 192.168.40.2:52444 (192.168.51.1:12345) -> 142.250.185.78:443       ESTABLISHED:ESTABLISHED',
      '   age 00:01:00, 10:10 pkts, 5000:3000 bytes, rule 1',
    ].join('\n');
    const samples = parsePfStates(out);
    const dev = samples.find((s) => s.ip === '192.168.40.2');
    expect(dev?.topPeers?.[0]?.ip).toBe('142.250.185.78');
    expect(dev?.topPeers?.some((p) => p.ip === '192.168.51.1')).toBe(false);
  });
});

describe('parseRestFirewallStates', () => {
  it('parses endpoint IPs and aggregates LAN → WAN peers', () => {
    expect(ipFromPfEndpoint('192.168.51.100:52344')).toBe('192.168.51.100');
    const samples = parseRestFirewallStates([
      {
        source: '192.168.51.100:52344',
      ...
      {
        source: '8.8.8.8:53',
        destination: '192.168.51.100:41000',
        direction: 'in',
        bytes_in: 2565,
        bytes_out: 1217,
        bytes_total: 3782,
  ...
  it('ignores LAN-only peers from scanner noise', async () => {
    ...
    expect(
      trafficSuggestsAlive({
        ...
        rateBps: 0,
        connections: 2,
        topPeers: [{ ip: '192.168.51.106', bytes: 168 }],
      }),
    ).toBe(false);
  });
